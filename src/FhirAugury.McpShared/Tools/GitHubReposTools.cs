...
public static class GitHubReposTools
{
    ...
    [McpServerTool, Description("Get the details of a single GitHub repository by owner and name.")]
    public static async Task<string> GetGitHubRepo(
        IHttpClientFactory httpClientFactory,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            HttpClient client = httpClientFactory.CreateClient("orchestrator");
            string url = $"/api/v1/github/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(name)}";
