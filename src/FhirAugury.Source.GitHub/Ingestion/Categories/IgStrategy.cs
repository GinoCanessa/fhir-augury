...
/// <summary>
/// Strategy for Implementation Guide (IG) repositories.
/// Stub implementation for Phase 1; validates via IG project markers.
/// </summary>
public class IgStrategy(
    ArtifactFileMapper artifactFileMapper,
    ILogger<IgStrategy> logger) : IRepoCategoryStrategy
{
    public RepoCategory Category => RepoCategory.Ig;
    public string StrategyName => "ig";
        ...
        string repoFullName, string clonePath, CancellationToken ct)
    {
        ...
        logger.LogInformation("Discovered {Count} FSH files in IG {Repo}", fshFiles.Count, repoFullName);
