...
public static class CommandDispatcher
{
    ...
    private static readonly Dictionary<string, Func<JsonElement, CliRequest>> Parsers = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["snapshot"] = j => j.Deserialize<SnapshotRequest>(DeserializeOptions)!,
        ["related"] = j => j.Deserialize<RelatedRequest>(DeserializeOptions)!,
        ["xref"] = j => j.Deserialize<XrefRequest>(DeserializeOptions)!,
    ...
    private static Task<object> DispatchAsync(CliRequest request, string orchestratorAddr, CancellationToken ct) =>
        request switch
        {
            ...
            SnapshotRequest r => SnapshotHandler.HandleAsync(r, orchestratorAddr, ct),
            RelatedRequest r => RelatedHandler.HandleAsync(r, orchestratorAddr, ct),
            XrefRequest r => XrefHandler.HandleAsync(r, orchestratorAddr, ct),
