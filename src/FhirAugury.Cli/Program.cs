...
public static class Program
{
    ...
    {
        RootCommand rootCommand = new RootCommand("FHIR Augury — unified knowledge platform CLI");

        // Global options
        Option<string> orchestratorOption = new Option<string>("--orchestrator")
        {
            Description = "Orchestrator gRPC address",
            DefaultValueFactory = _ =>
                Environment.GetEnvironmentVariable("FHIR_AUGURY_ORCHESTRATOR") ?? "http://localhost:5151",
        };
        Option<string> formatOption = new Option<string>("--format")
        {
        ...
        Option<bool> versionOption = new Option<bool>("--version")
        {
