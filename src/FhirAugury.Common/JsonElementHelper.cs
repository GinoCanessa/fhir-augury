...
public static class JsonElementHelper
{
    ...
    {
        if (!parent.TryGetProperty(prop1, out JsonElement p1) || p1.ValueKind == JsonValueKind.Null) return null;
        if (!p1.TryGetProperty(prop2, out JsonElement p2) || p2.ValueKind == JsonValueKind.Null) return null;
    ...
    public static string? GetNestedString(JsonElement parent, string prop1, string prop2, string prop3)
    {
        if (!parent.TryGetProperty(prop1, out JsonElement p1) || p1.ValueKind == JsonValueKind.Null) return null;
        if (!p1.TryGetProperty(prop2, out JsonElement p2) || p2.ValueKind == JsonValueKind.Null) return null;
        if (!p2.TryGetProperty(prop3, out JsonElement p3) || p3.ValueKind == JsonValueKind.Null) return null;
