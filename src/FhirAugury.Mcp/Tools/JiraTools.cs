...
public static class JiraTools
{
        ...
        [Description("Maximum results (default 20)")] int limit = 20)
    {
        ...
        var response = await jiraSource.SearchAsync(request);
        ...
        [Description("Maximum comments (default 50)")] int limit = 50)
    {
        ...
        await foreach (var comment in call.ResponseStream.ReadAllAsync())
        {
        ...
        [Description("Maximum results (default 20)")] int limit = 20)
    {
        ...
        using var call = jira.QueryIssues(request);
        ...
        var count = 0;
        await foreach (var issue in call.ResponseStream.ReadAllAsync())
        {
        ...
        [Description("Filter by work group")] string? workGroup = null)
    {
        ...
        using var call = jiraSource.ListItems(request);
