...
public class CacheFileNamingTests
{
    ...
    {
        bool result = Caching.CacheFileNaming.TryParse("DayOf_2025-01-15-001.xml", out CacheFileNaming.ParsedBatchFile? parsed);
        Assert.True(result);
    ...
    {
        bool result = Caching.CacheFileNaming.TryParse("_WeekOf_2025-01-13-000.json", out CacheFileNaming.ParsedBatchFile? parsed);
        Assert.True(result);
    ...
    {
        CacheFileNaming.ParsedBatchFile[] files = new[]
        {
        ...
        List<CacheFileNaming.ParsedBatchFile> sorted = Caching.CacheFileNaming.SortForIngestion(files).ToList();
