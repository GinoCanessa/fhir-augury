...
public class CrossRefPatternsTests
{
    ...
    {
        var links = CrossRefPatterns.ExtractLinks("See FHIR-43499 for details");
        Assert.Single(links);
    ...
    {
        var links = CrossRefPatterns.ExtractLinks("Check https://jira.hl7.org/browse/FHIR-12345");
        Assert.Single(links);
        Assert.Equal("jira", links[0].TargetType);
    ...
    {
        var links = CrossRefPatterns.ExtractLinks("See https://github.com/HL7/fhir/issues/42");
        Assert.Single(links);
    ...
    {
        var links = CrossRefPatterns.ExtractLinks("Related to HL7/fhir#823");
        Assert.Single(links);
        Assert.Equal("github", links[0].TargetType);
    ...
    {
        var links = CrossRefPatterns.ExtractLinks(
    ...
    {
        var links = CrossRefPatterns.ExtractLinks("No cross references here");
        Assert.Empty(links);
    }
}
