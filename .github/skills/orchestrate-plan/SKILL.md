...
match is found. The orchestrator surfaces an
**Ambiguous-selector** error (409 from jira-source) when two or more
catalog rows tie within the ambiguity delta.

- **Use `nameClean` verbatim for output subdirectory names** — folder
  paths must stay stable across runs. Folder slugs are generated by
  `Hl7WorkGroupNameCleaner.Clean` and surface in the response under
  `nameClean`.
- **Use `name` for human-facing labels** — README headings, run
  reports, and skill prose all render the display name. Folder
  slugs only appear in URLs / file paths.
- **`catalogJoinDegraded` is a proceed-with-warning signal.** Show a
  single top-of-run banner when set; do not abort. Every entry's
  `nameClean` is still populated — the jira-source server falls back
  to `Hl7WorkGroupNameCleaner.Clean(name)` when the HL7 catalog join
  is empty. The single exception is `selector=all` combined with the
  destructive `wipe-first` replace mode, which requires an explicit
  override (`allowDegradedWipeAll=true`) per D4.
